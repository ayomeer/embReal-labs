...
protected:
	...
	struct buffer_type {
		static constexpr size_type size = 256U; //size of ringbuffer (bytes)
	...
	static void* do_allocate(size_type chunk_size) {
		...
		const std::uint_fast8_t misaligned_amount(
				chunk_size % buffer_alignment);
...
// --- Wrapped Classes ----------------------------------------------------------
template<typename T, const std::uint_fast8_t buffer_alignment = UINT8_C(16)>
...
template<typename T, const std::uint_fast8_t buffer_alignment>
...
public:
	...
	ring_allocator(const ring_allocator&) noexcept :
			ring_allocator_base(ring_allocator()) {
	}
	...
	pointer allocate(size_type count,
			typename ring_allocator<void, buffer_alignment>::const_pointer =
					nullptr) {
